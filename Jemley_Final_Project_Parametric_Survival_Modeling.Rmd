---
title: "Case_Study_Parametric_Survival_Modeling"
author: "Peter Jemley"
date: "4/3/2020"
output:
...
- \usepackage{dcolumn}
---
# Test Text added for commit
#Final Project, DA 5030 Data Mining & Machine Learning, Peter Jemley

# Case Study Introduction: Prognostic Nomograms from Parametric Survival Modeling

\begin{tcolorbox}[width=\textwidth, colback={yellow}]
For the final project in one of my graduate bioinformatics courses I created a nomogram to graphically represent predicted mean and median survival time-windows modeled on biomedical and demographic data taken from the acutely-ill patients in the SUPPORT Study dataset archived in Vanderbilt University’s Department of Biostatistics. \\
...
```{r}
# Scrub the environment, load libraries, and attach required special 
# transformation functions from the {rms} package, then scrub the console:
...
```{r}
# Retrieve the study's data:
getHdata(support)

# Flagging categories of interest:
...
\begin{tcolorbox}[width=\textwidth, colback={yellow}]
Showing patterns in any missing data\\
...
So we can see that the two variables whose values are the most similar to each other are serum creatinine and serum bun — both renal function measures.\\

This is the closest we come to having two variables which may be redundant.
\end{tcolorbox}

```{r detect_statistical_associations}
# I create a new data subset for the modeling. I then drop the unused levels from
# support's designated acute illness groups, and label the new subset Disease 
# Group.
...
# Create the label and apply it:
label(ac$dzgroup) <- 'Disease Group'

# Then I attach the new subset to R's search path before implementing the
# varclus() function. Then I plot the similarity matrix created by varclus() as a
...
```{r}
# For age, a numeric variable:
boxplot(ac$age, xlab = "Age")

# For average TISS (avtisst, the Therapeutic Intervention Scoring System, 
#which is a method for calculating costs in the intensive care unit (ICU) and #intermediate care unit (IMCU) of a hospital), a numeric variable:
boxplot(ac$avtisst, xlab = "TISS")

# For mean arterial blood pressure, a numeric variable:
boxplot(ac$meanbp, xlab = "Mean Blood Pressue")

# For heart rate, a numeric variable. As plotted, heart rate appears to 
# have outliers. These rates are not unusual for patients with supraventricular #tachycardia, however. 
boxplot(ac$hrt, xlab = "Heart Rate")

# For respiration rate, day 3, a numeric variable. Tachypnea, rapid breathing, 
# is also not unusual in acutely-ill patients. The patient with the rate 
# which is the outlier died.
boxplot(ac$resp, xlab = "Respiration")

# For temperature (celcius), day 3, a numeric variable. The one outlier was 
# also associated with patient who died.
boxplot(ac$temp, xlab = "Temperature")

# For pafi, a measure of oxygenation of tissues, a numeric variable. The 
# greatest outlier is over the reference range (5 - 800), but this value 
# should remain as it may in fact be an accurate measurement. 
boxplot(ac$pafi, xlab = "PAFI")

# For serum albumin (alb), a numeric variable. All of these values are within 
# the reference range for acutely-ill patients.
boxplot(ac$alb, xlab = "Albumin")

# For bilirubin day 3 (bili), a numeric variable. Although these outliers are 
# extreme, they are noted in the medical literature for acutely-ill patients.
boxplot(ac$bili, xlab = "Bilirubin")

# For serum creatinine day 3 (crea), a numeric variable. These outliers 
# indicate renal failure, and would be part of the Multiple Organ System 
# Failure disease group.
boxplot(ac$crea, xlab = "Serum Creatinine")

# For serum sodium day 3 (sod), a numeric variable. These outliers are also
# within the reference range for acutely-ill patients.
boxplot(ac$sod, xlab = "Serum Sodium")

# For serum pH arterial day 3 (ph), a numeric variable. These outliers are
# also within the reference range for acutely-ill patients.
boxplot(ac$ph, xlab = "pH")

# For glucose day 3 (glucose), a numeric variable. These outliers are within
# the reference range for diabetic patients.
boxplot(ac$glucose, xlab = "Glucose")

# For blood urea nitrogen day 3 (bun), a numeric variable. These outliers are
# also within the reference range for acutely-ill patients.
boxplot(ac$bun, xlab = "Blood Urea Nitrogen")

# For urine output day 3 (urine), a numeric variable. The outliers are
# excessive, but appear in the medical literature.
...
Using the R {rms} package I create the survival time object which is a 2-column variable. It has the years of follow up or years until death, and the second column is binary indicator of what type of follow up that was, follow up while still alive, or at death.\\

Next I made stratified Kaplan-Meier Estimates, transforming the y-axis using the inverse normal transformation/or Z transformation. The plots' linearities and approximate-parallelisms demonstrate a reasonable fit for the log-normal accelerated failure time model using one predictor.\\

Then I fit a parametric survival model of type log normal accelerated failure time model, using disease group as a predictor, a restricted cubic spline for age with 5 knots, and the same for blood pressure with 5 knots.\\
...
There are 3 very powerful variables in this tentative model, and by the time one has 3 powerful variables the residuals should behave similarly to the way they will in a final model.\\
\end{tcolorbox}

```{r tentative_model_analysis}
# Creating distribution summaries for predictor variables using the 
# datadist() function {rms}:
dd <- datadist(ac)

# Describing the distributions of variables to {rms}
options(datadist = 'dd')

# Converting the days of follow up (d.time) variable to years:
years <- d.time / 365.25
units(years) <- 'Year'

# Creating the right-censored survival time object using the Surv() function
# {survival}:
S <- Surv(years, death)

# Displaying Kaplan-Meier estimates stratified by disease group (dzgroup) using
# the survplot() function {rms}.
# Show normal inverse Kaplan-Meier estimates stratified by dzgroup
...
To help with this I generate a random number which can serve as a touchstone for the tentative model. When stratifying by a random number the sampling variation will still be there but it will be pure sampling variation because a random number is unrelated to anything in the dataset — stratifying by a random number should generate identical distributions as those in the data.\\
...
When I stratify by disease group there's a little bit of a shoulder there early on which is slightly problematic. Stratifying by age is a very good fit: all of those 4 curves by quartiles of age are hovering around the thick black curve which is the theoretical 1 minus normal cumulative distribution function that should hold if our model assumptions are true.\\
...
Like all graphical assessments of model fit this is a subjective process but this model fit is as good as one usually sees.\\
\end{tcolorbox}

```{r tentative_psm_model_residuals}
# Creating a parametric survival model object using the psm() function {rms}.
# The object S is created as a function of the disease group, a restricted cubic
# spline for age with 5 knots, and the same for mean arterial blood pressure 
# with 5 knots.
...
# Extracting the model's residuals using the residuals() function {stats}:
r <- resid(f)

# Plotting the curves of the residuals: 
survplot(r, dzgroup, label.curve = FALSE)
survplot(r, age,     label.curve = FALSE)
survplot(r, meanbp,  label.curve = FALSE)

# Creating the random variables and plotting them:
...
\begin{tcolorbox}[width=\textwidth]
Next I begin building the accelerated failure time parametric survival model. A big part of building this model is getting the subject matter input about which of the variables should be used as predictors and another big part is how to assign degrees of freedom to those variables. \\
...
I examined which variables had enough predictive potential to justify giving them many of degrees of freedom, i.e. variables which were very steep in their relationship to survival time. You have to try to avoid under-modeling such variables by assuming they're linear when they may not be.\\

I began by using the pairwise Spearman rho squared method: the generalized one that uses the rank of the predictor and the square of the rank of the predictor against the rank of survival time.\\

Spearman’s rho squared doesn’t know anything about censoring so you can’t give it the time until event variable because its censored values would be considered the same as the uncensored values, which is a clear error.\\

From the data I found that the shortest follow up time for survivors is a year, so I was able to truncate the data at one year and just look at the action between zero and 365 days. That is the period for which there is complete data, and for any patient followed for 365 days we know that they were either alive at day 365 or we know the date of their death within that interval.\\

This provides complete data, but data with a substantially lower information content because we’re not distinguishing between a death in the truncated data and any death between 366 days and 1000 days.\\
...
The coma score is a bit different because it was derived from an overall analysis of the SUPPORT patients, and although derived from a Cox model it was linear. So I had to end up giving one degree of freedom to that even though that knowledge was gained from an analysis other than this one.\\

The mean blood pressure is continuous so we want to give it the max number of knots of later, and for disease group we want to leave in its three categories and not collapse any of those. For the variables with lower potential prediction value they will have only one degree of freedom, forcing them to be linear.\\

This type of analysis, as well as the next one we’re going to do, is looking at predictors one variable at a time, not each adjusted for the other, making this analysis somewhat unreliable.
\end{tcolorbox}

```{r first_tentative_psm_analysis}
# Calculating the shortest follow up:
shortest.follow.up <- min(d.time[death == 0], na.rm = TRUE)

# Converting shortest_follow_up to an object for the spearman2() function
# a parallel minima object:
d.timet <- pmin(d.time, shortest.follow.up)

# Creating the bivariate Spearman’s rho squared object with 2 degrees of freedom:
w <- spearman2(
...
\begin{tcolorbox}[width=\textwidth]
I was able to use a different rank correlation coefficient, namely Somer’s Dxy, which has been adapted to handle right censoring. When calculating concordant and discordant numbers that go into all possible pairs analysis with Somer’s D we can take partial information into account.\\

For example, if someone died before someone else was censored we know that the censored person has already outlived the other person and we know how the ranking between them works in that sense. But the analysis includes two persons who were censored and we cannot compare them, so when calculating concordant pairs that will be one uncertain pair.\\

So, I'm taking into account censoring and looking at the variables one at a time using Somer’s Dxy instead of using a Spearman correlation.  Although Somer’s Dxy is not able to detect nonmonotonic relationships (variables which do not generally vary in the same way or direction), when we use this approach, using the rcorrcens() function in the {Hmisc} package this makes it easy to take the censoring discussed immediately above into account.\\

This results in the graph in which we can see that mean blood pressure has the highest prediction potential, creatinine next, then disease group, then coma score: they’re all very high.\\

We can have more confidence in this graph than the previous one which used the oversimplified Spearman’s rho squared.\\
\end{tcolorbox}

```{r second_tentative_psm_analysis}
# Taking censoring into account using the rcorrcens() function {Hmisc}:
w <- rcorrcens(
...
# Simple Imputation
\begin{tcolorbox}[width=\textwidth]
Here is code for simple imputation based on sound statistical principles.\\

For example, if white blood count data is missing I entered 9 (which in the original scale was 9000). For this type of imputation there was a federal grant from what is now called AHRQ to study the safest ways to impute data in an ICU population like this.\\
...
# Collapse race into race2 as race turned out not to be relevant in this dataset:
levels(race2) <- list(white = 'white', other = levels(race)[-1])
race2[is.na(race2)] <- 'white'

# datadist() determines summaries of variables for effect and plotting ranges,
# values to adjust to, and overall ranges for the functions Predict, plot.Predict,
# ggplot.Predict, summary.rms, survplot, and nomogram.rms.
...
```{r fit_psm_max_4}
# Creating a constant to represent 4 knots for the psm() function {rms}:
k <- 4

# Using "rcs" {rms} helps to blind researchers from becoming aware of the 
# linear forms of continuous variables' effect, removing a source of bias:
f <- psm(
...
# Plot the ANOVA — the plot blinds analysts from seeing the forms of 
# coefficients' effects, i.e. linearity test result:
plot(anova(f)) 
```
...
# Plotting the ANOVA
\begin{tcolorbox}[width=\textwidth]
Plotting the ANOVA is where we see that disease group is the biggest predictor. This is similar to before when the model was saturated and I assumed more degrees of freedom for some of the variables.
...
```{r validate_indexes}
# Adding data to the fitted model to allow the bootstrap to re-sample:
...
```{r}
# calibrate() {rms} uses bootstrapping or here to get bias-corrected, or 
# overfitting corrected, estimates of predicted vs. observed values based on
# subsetting predictions into intervals (for survival models) or on
# nonparametric smoothers (for other models).
...
# Here you use sigma = 1 to prevent sigma hat from being zerowhen R2 = 1,
# which is the case here because I began by approximating Z with all 
# the variables from the full model:
a <- ols(
...
# fastbw() {rms} performs a numerically stable version of fast backward 
# elimination on factors. It uses a fitted complete model and 
...
```{r}
# This returns the variance-covariance matrix of the main parameters of a
# fitted model object — in this case the full model, f_2. 
# var(full model)
V <- vcov(f_2, regcoef.only = TRUE)

# This creates the design of the full model:
X <- cbind(Intercept = 1, g$x)

# This creates the design of the approxmiate model:
x <- cbind(Intercept = 1, f.approx$x)

# This creates a contrast matrix for the full and approximate models:
w <- solve(t(x) %*% x, t(x)) %*% X

# This formula maps the full model coefficients into the approximate 
# model coefficients.
v <- w %*% V %*% t(w)
```
...
```{r}
# Here you compare the variance estimates — the diagonals of v created 
# immediately above — with variance estimates from the reduced model ﬁtted
...
# diag() {base} extract or replace the diagonal of a matrix, or construct 
# a diagonal matrix. Here it constructs a diagonal matrix
...
$$ E(Z) = X\beta,\ \ where$$
$$ X\hat{\beta}=$$
$$ -2.51 \\-1.94[{\rm Coma}]-1.75[{\rm MOSF\ w/Malig}]\\ $$



# Getting a simpler representation of the model by applying the nomogram() function to the approximate model
\begin{tcolorbox}[width=\textwidth]
