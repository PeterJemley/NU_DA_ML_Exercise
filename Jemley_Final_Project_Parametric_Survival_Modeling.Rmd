...
\end{tcolorbox}
\begin{tcolorbox}[width=\textwidth]
1. predict() from the stats package (with a lower case p) is a generic function for predictions from the results of various model fitting functions.\\
$$ X*\hat{\beta} $$ 
...
5. This formula maps the full model's coefficients onto the approximate model’s coefficients.
\end{tcolorbox}
```{r}
# 1
...
\begin{tcolorbox}[width=\textwidth, colback={yellow}]
In the results you can note the similarities between these statistics and those from the  Wald statistics table for the full model. Had they been very collinear with the kept/retained variables these similarities would not be the case.
\end{tcolorbox}
...
\end{tcolorbox}
\begin{tcolorbox}[width=\textwidth, colback={yellow}]
Note: The nomogram and a few other plots render somewhat poorly in a PDF. When printed or viewed in the plot viewer, however, their formatting is correct.
\end{tcolorbox}
